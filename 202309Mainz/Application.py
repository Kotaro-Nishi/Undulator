...
def fit_button_clicked():
    Folder = loadstring.get() #folder identification 
    NumOfSeries = int(NumOfSeries_string.get()) #NumOfSeries = 825(default) 
    LengthOfData = int(LengthOfData_string.get()) #length of data = 825(default)

    Interval = [int(Interval_string1.get()), int(Interval_string2.get())] #interval 0-825
    ...
    MoFu.LambdaRfit = lambdaR #4.12e-7
    MoFu.dispersion = float(dispersion_string.get()) #4.8088e-12
    MoFu.pixelof404nm = float(pixelof404nm_string.get()) #1315.3024
...
def open_tif_viewer():
    subprocess.call("python3.9 TifViewer.py", shell=True)

def data_button_clicked():
    thread = threading.Thread(target=open_tif_viewer)
    thread.start()

def open_draw_modular():
    #DrawModular.draw_modular_app()
    subprocess.call("python3.9 DrawModular_App.py", shell=True) 
    #subprocess.Popen(["python3.9", "DrawModular_App.py"])
...
def draw_button_clicked():
    ...
    Amplfit = float(Guess_string1.get())
    Ytran = float(Guess_string2.get())
    gammafit = float(Guess_string3.get())
    delta0fit = float(Guess_string4.get())
    zpfit = float(Guess_string5.get())
    elevationfit =float(Guess_string6.get())
    cfit = float(Guess_string7.get())
    ...
    #exec(open("DrawModular_App.py").read()) 
    thread = threading.Thread(target=open_draw_modular)
    thread.start()
...
data_button.grid(column=3,row=1, sticky="w", **options)
data_button.configure(command=data_button_clicked)

# draw button
draw_button = ttk.Button(frame,text = "Draw")
draw_button.grid(column=3,row = 2,sticky ="w", **options)
draw_button.configure(command=draw_button_clicked)
